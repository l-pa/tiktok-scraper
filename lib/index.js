...
class TikTokScraper extends EventEmitter {
    constructor({ download, filepath, filetype, proxy, cli, event, timeout, input, number, type, by_user_id = false, user_data = false, signature }) {
        super();
    ...
    async scrape() {
        ...
        const signature = await this.mainLoop();
        ...
        try {
            return { collector: this.collector, signature };
        } catch (error) {
            throw String(error);
        }
    }
